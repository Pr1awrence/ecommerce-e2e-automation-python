...
BUG_16_URL_CHANGE = "https://github.com/Pr1awrence/ecommerce-e2e-automation-python/issues/16"
...
def test_login_with_invalid_credentials(page, auth_page, home_page):
    ...
    expect(auth_page.login_form.error_message).to_have_text(INCORRECT_EMAIL_OR_PASSWORD_ERROR_TEXT)
...
def test_initial_signup_successful(page, auth_page, signup_page, home_page):
    page.route("**/api/signup", lambda route: route.fulfill(
        status=200,
        body='{"responseCode": 200, "message": "smth"}'
    ))
